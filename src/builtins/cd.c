...
/*   Updated: 2025/03/19 17:17:10 by nolecler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int exec_cd(t_cmd *cmd, t_pid_data *pdata, t_global_data *sdata)
{
	...
	temp_env = convert_env(sdata->envp);
	if (!temp_env)
		return (1); // FAUT TOUT METTOYER
	old_pwd = get_old_pwd(temp_env);// malloc qui n est pas free encore
	path = NULL;
	new_pwd = NULL;
	home = NULL;
	if (pdata->nb_cmd == 1)
	{
		if (old_pwd) // a voir , test a faire sans et avec;
			update_old_pwd(temp_env, old_pwd);
        ...
        if (new_pwd)
        {
			update_pwd(temp_env, new_pwd);
            free(new_pwd);
        }
	}
	clear_env_array(temp_env);
	free (pwd);
	free (old_pwd);
	return (0);
}
